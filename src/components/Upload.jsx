  ...
  const handleImageDiagnose = async (e) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        const response = await axios.post(
          ...
          },
          {
            headers: {
              ...
              "Api-Key": "OgxWQ2QUfQ2WZ7br07Ujb4hjQsSFlCRnPb2PHWBQBUT4b1wKjo", // Replace with your actual API key
            },
          }
        );
        ...
        const disease = response.data?.result?.disease;
        const crop = response.data?.result?.crop;
        const treatment = disease?.suggestions ? disease.suggestions[0]?.treatment : "No treatment available";
  
        if (disease && crop) {
          setResult({
            disease: disease.name || "Unknown disease",
            plant: crop.name || "Unknown plant",
            remedy: treatment || "No treatment available",
            status: "Infected",
          });
        } else {
          setResult({
